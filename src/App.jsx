...
import { Risk, Timeline, Employees, Requests, Reports, Notifications, SettingsPage, SaaSAdmin, TenantSwitcher, Procurement, Contracts, Assets, ExecutiveBrief, DepartmentHub, DocumentCenter, SmartForms, EmployeeApp } from './pages/MorePages';
...
export default function App(){
  ...
  return <div className="app" style={{'--primary':settings.primaryColor,'--secondary':settings.secondaryColor,'--accent':settings.accentColor,'--bgImage':settings.backgroundImage?`url(${settings.backgroundImage})`:'none'}}>
    ...
    <div className="layout"><Sidebar tab={tab} setTab={setTab}/><main>
      {tab==='dashboard'&&<Dashboard/>}{tab==='departments'&&<DepartmentHub/>}{tab==='documents'&&<DocumentCenter/>}{tab==='smartForms'&&<SmartForms/>}{tab==='employeeApp'&&<EmployeeApp/>}{tab==='ownerAI'&&<OwnerAI/>}{tab==='aiAssistant'&&<AIAssistant/>}{tab==='liveMap'&&<LiveMap/>}{tab==='risk'&&<Risk/>}{tab==='timeline'&&<Timeline/>}{tab==='employees'&&<Employees/>}{tab==='attendance'&&<Attendance/>}{tab==='payroll'&&<Payroll/>}{tab==='requests'&&<Requests/>}{tab==='procurement'&&<Procurement/>}{tab==='contracts'&&<Contracts/>}{tab==='assets'&&<Assets/>}{tab==='executiveBrief'&&<ExecutiveBrief/>}{tab==='reports'&&<Reports/>}{tab==='notifications'&&<Notifications/>}{tab==='settings'&&<SettingsPage/>}{tab==='saasAdmin'&&<SaaSAdmin/>}
