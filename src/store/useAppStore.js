...
const baseTheme = {
  brandSlogan: 'مركز القيادة الذكي',
...
export const useAppStore = create(persist((set, get) => ({
  ...
  attendance: [], requests: [], payroll: [], payrollMovements: [], suppliers: seedSuppliers, purchaseOrders: [], assets: seedAssets, contracts: seedContracts, documents: [], executiveBriefs: [], notifications: [], audit: [], whatsappQueue: [], errors: [],
  ...
  upsertDocument: (doc) => { if(!doc.title?.trim()) throw new Error('اسم المستند مطلوب'); const tid=get().currentTenantId; const emp=get().employees.find(e=>e.id===doc.employeeId); const rec={...doc, id:doc.id||uid('DOC'), tenantId:tid, employeeName:emp?.name||doc.employeeName||'', uploadDate:doc.uploadDate||today(), status:(doc.expiryDate&&doc.expiryDate<today())?'expired':(doc.expiryDate&&doc.expiryDate<=(()=>{const d=new Date();d.setDate(d.getDate()+30);return d.toISOString().slice(0,10)})())?'expiring':'active'}; set(st=>({documents: doc.id ? (st.documents||[]).map(x=>x.id===doc.id?rec:x) : [rec,...(st.documents||[])]})); get().addAudit('حفظ مستند', {title:rec.title, employee:rec.employeeName}); return rec; },
  deleteDocument: (id) => { set(st=>({documents:(st.documents||[]).filter(x=>x.id!==id)})); get().addAudit('حذف مستند', {id}); },
  generateDocumentAlerts: () => { const d=get().getTenantData(); const t=today(); const future=new Date(); future.setDate(future.getDate()+30); const soon=future.toISOString().slice(0,10); const alerts=(d.documents||[]).filter(x=>x.expiryDate&&x.expiryDate<=soon).map(x=>({id:uid('NOT'),tenantId:get().currentTenantId,date:t,time:new Date().toLocaleTimeString('ar-SA'),title:'تنبيه انتهاء مستند',message:`${x.title} ${x.employeeName?`للموظف ${x.employeeName}`:''} ينتهي بتاريخ ${x.expiryDate}`,read:false,type:'document'})); if(alerts.length) set(st=>({notifications:[...alerts,...(st.notifications||[])]})); return alerts; },
