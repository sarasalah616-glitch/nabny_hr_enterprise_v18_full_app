...
const baseTheme = {
  brandSlogan: 'NABNY Executive Intelligence',
  ...
  workDays: [0,1,2,3,4], payrollBasis: 'workDays', overtimeRate: 1.5,
  defaultProjectRadius: 120, requireGps: false,
  whatsappApiUrl: '', whatsappToken: '',
  punchOpen:'06:00', punchClose:'10:00', punchOutOpen:'15:00', punchOutClose:'22:00',
  documentAlertDays:60, defaultLanguage:'ar',
};
const defaultSettings = { systemName: 'NABNY OS v25 Enterprise Command Center', company: 'شركة نبني للمقاولات', ...baseTheme };
...
const seedEmployees = [
  { id:'EMP-1001', tenantId:'TEN-NABNY', name:'أحمد محمد', role:'مشرف موقع', department:'التشغيل', projectId:'PRJ-JED', basicSalary:5000, startDate:today(), status:'active', phone:'0550000001', iqamaNo:'2500000001', iqamaExpiry:'2026-07-20', passportNo:'A10001', passportExpiry:'2027-01-15', files:[] },
  { id:'EMP-1002', tenantId:'TEN-NABNY', name:'خالد علي', role:'مندوب مشتريات', department:'المشتريات', projectId:'PRJ-JED', basicSalary:4500, startDate:today(), status:'active', phone:'0550000002', iqamaNo:'2500000002', iqamaExpiry:'2026-06-25', passportNo:'A10002', passportExpiry:'2026-10-30', files:[] },
...
const seedDepartments = [
  { id:'DEP-MGMT', tenantId:'TEN-NABNY', name:'الإدارة', lead:'المدير التنفيذي', budget:0, status:'active' },
  { id:'DEP-HR', tenantId:'TEN-NABNY', name:'الموارد البشرية', lead:'مدير الموارد البشرية', budget:0, status:'active' },
  { id:'DEP-FIN', tenantId:'TEN-NABNY', name:'الإدارة المالية', lead:'المدير المالي', budget:0, status:'active' },
  { id:'DEP-PUR', tenantId:'TEN-NABNY', name:'المشتريات', lead:'مدير المشتريات', budget:0, status:'active' },
  { id:'DEP-CON', tenantId:'TEN-NABNY', name:'الإنشاءات', lead:'مدير الإنشاءات', budget:0, status:'active' },
  { id:'DEP-MNT', tenantId:'TEN-NABNY', name:'الصيانة', lead:'مدير الصيانة', budget:0, status:'active' },
  { id:'DEP-OPS', tenantId:'TEN-NABNY', name:'التشغيل', lead:'مدير التشغيل', budget:0, status:'active' },
];
const seedDocuments = [
  { id:'DOC-1001', tenantId:'TEN-NABNY', employeeId:'EMP-1001', employeeName:'أحمد محمد', type:'إقامة', title:'إقامة أحمد محمد', expiryDate:'2026-07-20', status:'valid', fileName:'iqama-ahmed.pdf', fileData:'', uploadedAt:today() },
  { id:'DOC-1002', tenantId:'TEN-NABNY', employeeId:'EMP-1002', employeeName:'خالد علي', type:'جواز', title:'جواز خالد علي', expiryDate:'2026-10-30', status:'valid', fileName:'passport-khaled.pdf', fileData:'', uploadedAt:today() },
];
...
export const useAppStore = create(persist((set, get) => ({
  ...
  attendance: [], requests: [], payroll: [], payrollMovements: [], hrForms: [], documents: seedDocuments, departments: seedDepartments, monthlySnapshots: [], suppliers: seedSuppliers, purchaseOrders: [], assets: seedAssets, contracts: seedContracts, executiveBriefs: [], notifications: [], audit: [], whatsappQueue: [], errors: [], language:'ar',

  getTenant: () => get().tenants.find(t => t.id === get().currentTenantId) || get().tenants[0],
  getSettings: () => get().getTenant()?.settings || defaultSettings,
  getTenantData: () => { const s=get(), tid=s.currentTenantId; return { employees:(s.employees||[]).filter(x=>x.tenantId===tid), projects:(s.projects||[]).filter(x=>x.tenantId===tid), attendance:(s.attendance||[]).filter(x=>x.tenantId===tid), requests:(s.requests||[]).filter(x=>x.tenantId===tid), payroll:(s.payroll||[]).filter(x=>x.tenantId===tid), payrollMovements:(s.payrollMovements||[]).filter(x=>x.tenantId===tid), suppliers:(s.suppliers||[]).filter(x=>x.tenantId===tid), purchaseOrders:(s.purchaseOrders||[]).filter(x=>x.tenantId===tid), assets:(s.assets||[]).filter(x=>x.tenantId===tid), contracts:(s.contracts||[]).filter(x=>x.tenantId===tid), executiveBriefs:(s.executiveBriefs||[]).filter(x=>x.tenantId===tid), documents:(s.documents||[]).filter(x=>x.tenantId===tid), hrForms:(s.hrForms||[]).filter(x=>x.tenantId===tid), departments:(s.departments||[]).filter(x=>x.tenantId===tid), monthlySnapshots:(s.monthlySnapshots||[]).filter(x=>x.tenantId===tid), notifications:(s.notifications||[]).filter(x=>x.tenantId===tid), audit:(s.audit||[]).filter(x=>x.tenantId===tid) }; },
  ...
  upsertDepartment: (data) => { const tid=get().currentTenantId; if(!data.name?.trim()) throw new Error('اسم القسم مطلوب'); const rec={id:data.id||uid('DEP'),tenantId:tid,status:'active',lead:'',budget:0,...data,tenantId:tid,budget:toNum(data.budget)}; set(s=>({departments:(s.departments||[]).some(x=>x.id===rec.id&&x.tenantId===tid)?(s.departments||[]).map(x=>x.id===rec.id&&x.tenantId===tid?rec:x):[rec,...(s.departments||[])]})); get().addAudit(data.id?'تعديل قسم':'إضافة قسم',{department:rec.name}); return rec; },
  uploadEmployeeDocument: (data) => { const tid=get().currentTenantId; const emp=(get().employees||[]).find(e=>e.id===data.employeeId&&e.tenantId===tid); if(!emp) throw new Error('اختر موظف'); if(!data.type) throw new Error('نوع الوثيقة مطلوب'); const rec={id:data.id||uid('DOC'),tenantId:tid,employeeId:emp.id,employeeName:emp.name,type:data.type,title:data.title||`${data.type} - ${emp.name}`,expiryDate:data.expiryDate||'',status:'valid',fileName:data.fileName||'',fileData:data.fileData||'',uploadedAt:today(),notes:data.notes||''}; set(s=>({documents:(s.documents||[]).some(x=>x.id===rec.id&&x.tenantId===tid)?(s.documents||[]).map(x=>x.id===rec.id&&x.tenantId===tid?rec:x):[rec,...(s.documents||[])]})); get().addAudit('رفع وثيقة موظف',{employee:emp.name,type:rec.type}); if(rec.expiryDate)get().scanDocumentAlerts(); return rec; },
  scanDocumentAlerts: () => { const s=get(), tid=s.currentTenantId, settings=s.getSettings(); const limit=new Date(); limit.setDate(limit.getDate()+toNum(settings.documentAlertDays||60)); const limitKey=dateKey(limit); const docs=(s.documents||[]).filter(d=>d.tenantId===tid&&d.expiryDate&&d.expiryDate<=limitKey); docs.forEach(doc=>{ const exists=(s.notifications||[]).some(n=>n.tenantId===tid&&n.docId===doc.id&&n.title.includes('انتهاء وثيقة')); if(!exists)get().addNotification({title:'تنبيه انتهاء وثيقة',message:`${doc.type} للموظف ${doc.employeeName} تنتهي بتاريخ ${doc.expiryDate}`,level:doc.expiryDate<today()?'danger':'warning',docId:doc.id}); }); return docs; },
  createSmartHRForm: (data) => { const tid=get().currentTenantId; const emp=(get().employees||[]).find(e=>e.id===data.employeeId&&e.tenantId===tid); if(!emp) throw new Error('اختر موظف للنموذج'); const templates={loan:'طلب سلفة',vacation:'طلب إجازة',iqama:'تجديد إقامة',passport:'تجديد جواز',clearance:'إخلاء طرف',letter:'خطاب تعريف'}; const rec={id:uid('FRM'),tenantId:tid,type:data.type||'letter',title:templates[data.type]||data.type,employeeId:emp.id,employeeName:emp.name,department:emp.department,role:emp.role,projectId:emp.projectId,basicSalary:emp.basicSalary,iqamaNo:emp.iqamaNo||'',iqamaExpiry:emp.iqamaExpiry||'',passportNo:emp.passportNo||'',passportExpiry:emp.passportExpiry||'',amount:toNum(data.amount),fromDate:data.fromDate||'',toDate:data.toDate||'',notes:data.notes||'',status:'draft',date:today(),history:[{date:today(),action:'توليد تلقائي'}]}; set(s=>({hrForms:[rec,...(s.hrForms||[])]})); get().addAudit('توليد نموذج HR ذكي',{form:rec.title,employee:emp.name}); return rec; },
  updateHRForm: (id, patch) => { const tid=get().currentTenantId; set(s=>({hrForms:(s.hrForms||[]).map(f=>f.id===id&&f.tenantId===tid?{...f,...patch,history:[...(f.history||[]),{date:today(),action:patch.status||'تعديل'}]}:f)})); },
  archiveMonth: (ym=month()) => { const s=get(), tid=s.currentTenantId, d=s.getTenantData(); const snapshot={id:uid('MON'),tenantId:tid,month:ym,date:today(),attendance:d.attendance.filter(a=>a.month===ym).length,requests:d.requests.filter(r=>(r.date||'').startsWith(ym)).length,payroll:d.payroll.filter(p=>p.month===ym).reduce((sum,p)=>sum+toNum(p.net),0),documents:d.documents.length,employees:d.employees.filter(e=>e.status==='active').length}; set(st=>({monthlySnapshots:[snapshot,...(st.monthlySnapshots||[]).filter(x=>!(x.tenantId===tid&&x.month===ym))]})); get().addAudit('حفظ أرشيف شهري',{month:ym}); return snapshot; },
