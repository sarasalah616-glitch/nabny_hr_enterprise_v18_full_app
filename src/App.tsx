...
class ErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean; message: string }> {
  ...
  static getDerivedStateFromError(error: Error) {
    return { hasError: true, message: error?.message || 'حدث خطأ في تحميل الصفحة' };
  }
  render() {
      ...
      return (
        <div className="login">
          <div className="loginCard" style={{ textAlign: 'center' }}>
            <h2 style={{ marginBottom: 12 }}>تم منع الصفحة البيضاء</h2>
            <p className="muted" style={{ marginBottom: 16 }}>{this.state.message}</p>
            <button className="btn green" onClick={() => { localStorage.clear(); window.location.href = '/'; }}>إعادة ضبط وتشغيل النظام</button>
          </div>
        </div>
      );
    }
